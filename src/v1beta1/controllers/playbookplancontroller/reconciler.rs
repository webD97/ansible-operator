        ...
        match timing {
            Timing::Now => {
                ...
                for host in hosts_to_trigger {
                    let job = job_builder::create_job_for_host(&host, &execution_hash, &object)?;
                    let job_name = job
                        .name()
                    ...
                    // FIXME: For a recurring job, this does not really make sense. We need to check if the previous
                    //        one is still running and if not ensure a new unique name.
            ...
            Timing::Delayed(until) => {
                ...
                resource_status.next_run =
                    Some(until.to_rfc3339_opts(chrono::SecondsFormat::Secs, true));
            }
        };
    }
